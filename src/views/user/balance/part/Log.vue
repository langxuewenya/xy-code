        ...
        <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column prop="scene.text" label="余额变动场景">
            <template  #default="scope">
              ...
              <span v-if="scope.row.scene.value==50" style="color: #409EFF">{{scope.row.scene.text}}</span>
              <span v-if="scope.row.scene.value==60" style="color: #67C23A">{{scope.row.scene.text}}</span>
            </template>
          </el-table-column>
