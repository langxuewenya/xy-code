					...
					<el-table-column fixed="right" label="操作" width="150">
						<template #default="scope">
							<el-button v-if="scope.row.audit_status==0" @click="editClick(scope.row)" type="text"
								size="small" v-auth="'/audit/education/edit'">审核</el-button>
							<el-button v-if="scope.row.audit_status!=0" @click="detailClick(scope.row)" type="text"
								size="small" v-auth="'/audit/education/edit'">详情</el-button>
							<el-button @click="sectionClick(scope.row)" type="text" size="small"
								v-auth="'/audit/education/section'">课时</el-button>
						</template>
					</el-table-column>
				</el-table>
			</div>
	...
	export default {
		...
		methods: {
			...
			getData() {
				...
				self.loading = true;
				AuditApi.educationList(Params, true)
					.then(data => {
						self.loading = false;
