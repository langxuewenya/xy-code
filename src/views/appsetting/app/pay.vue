			...
			<el-form-item label="选择支付方式">
				<div v-for="(item, key, index) in platform" :label="item.value" :key="index">
					...
					<el-checkbox-group v-model="form.pay_type[key].pay_type">
						<el-checkbox v-for="(pay_item,pay_index) in item.pay_type" :label="pay_type[pay_item].value"
							:key="pay_index">{{pay_type[pay_item].name}}</el-checkbox>
					</el-checkbox-group>
				</div>
			</el-form-item>
