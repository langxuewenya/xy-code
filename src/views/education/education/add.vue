	...
	export default {
		...
		methods: {
			...
			getBaseData: function() {
				let self = this;
				EducationApi.getCourseBaseData({}, true)
					.then(res => {
						self.loading = false;
			...
			onSubmit: function() {
				let self = this;
				if(self.form.model.is_pay == 1 && self.form.active_type == 1){
				...
				self.$refs.form.validate(valid => {
					if (valid) {
						self.loading = true;
						EducationApi.addCourse(params, true)
							.then(data => {
								self.loading = false;
								ElMessage({
									message: '添加成功',
								...
								self.$router.push('/education/education/index');
							})
							.catch(error => {
								self.loading = false;
							});
