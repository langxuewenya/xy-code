<template>
	<div>
		...
		<el-form-item label="微信群：" :rules="[{ required: true, message: '请上传微信群' }]" prop="model.wx_image">
			<el-row>
				<div class="draggable-list">
					<div class="item img-select" v-if="form.model.wx_image == ''"
						@click="openProductUpload('image', 'wx')">
						<el-icon>
							<Plus></Plus>
						</el-icon>
					</div>
					<div v-if="form.model.wx_image != ''" class="item" @click="openProductUpload('image', 'wx')"><img
							:src="form.model.wx_image" width="100" height="100" /></div>
				</div>
			</el-row>
		</el-form-item>
		<el-form-item label="qq群：" :rules="[{ required: true, message: '请上传qq群' }]" prop="model.qq_image">
			<el-row>
				<div class="draggable-list">
					<div class="item img-select" v-if="form.model.qq_image == ''"
						@click="openProductUpload('image', 'qq')">
						<el-icon>
							<Plus></Plus>
						</el-icon>
					</div>
					<div v-if="form.model.qq_image != 0" class="item" @click="openProductUpload('image', 'qq')"><img
							:src="form.model.qq_image" width="100" height="100" /></div>
				</div>
			</el-row>
		</el-form-item>
		<!--其他设置-->
		<div class="common-form">其他设置</div>
		<el-form-item label="课程时间" :rules="[{ required: true, message: ' ' }]" prop="model.start_time">
			<el-date-picker v-model="form.model.start_time" type="date" value-format="YYYY-MM-DD" placeholder="选择开始时间">
			</el-date-picker>
			<span>-</span>
			<el-date-picker v-model="form.model.end_time" type="date" value-format="YYYY-MM-DD" placeholder="选择结束时间">
			</el-date-picker>
		</el-form-item>
		<el-form-item label="课时：" :rules="[{ required: true, message: ' ' }]" prop="model.class_time">
			<el-input type="number" min="0" v-model="form.model.class_time" class="max-w460"></el-input>
		</el-form-item>
		...
		methods: {
			...
			returnProductImgsFunc(e) {
				if (e != null) {
					...
					} else if (this.file_name == 'wx') {
						this.form.model.wx_image = e[0].file_path;
					} else if (this.file_name == 'qq') {
						this.form.model.qq_image = e[0].file_path;
					}
				}
