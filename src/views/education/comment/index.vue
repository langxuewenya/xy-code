					...
					<el-table-column label="课程" width="400px">
						<template #default="scope">
							<div class="product-info">
								<div class="pic"><img v-if="scope.row.course.image"
										v-img-url="scope.row.course.image.file_path" alt="" /></div>
								<div class="info">
					...
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button @click="detailClick(scope.row)" type="text" size="small"
								v-auth="'/education/comment/detail'">详情
							</el-button>
							<el-button @click="delClick(scope.row)" type="text" size="small"
								v-auth="'/education/comment/delete'">删除
							</el-button>
						</template>
					</el-table-column>
				</el-table>
	...
	export default {
		...
		methods: {
			...
			getData(param = '') {
				...
				EducationApi.getCommentList(Params, true).then(res => {
					self.loading = false;
			...
			onSubmit() {
				...
				Params.page = self.curPage;
				Params.list_rows = self.pageSize;
				EducationApi.getCommentList(Params, true).then(data => {
					self.loading = false;
