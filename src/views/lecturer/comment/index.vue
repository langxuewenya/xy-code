		...
		<div class="common-seach-wrap">
			<el-form size="small" :inline="true" :model="searchForm" class="demo-form-inline">
				...
				<el-form-item label="讲师名称">
					<el-input size="small" v-model="searchForm.name" placeholder="请输入讲师名称"></el-input>
				</el-form-item>
				<el-form-item>
		...
		<div class="product-content">
			<div class="table-wrap">
				...
				<el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
					<el-table-column label="讲师" width="200px">
						<template #default="scope">
							<div class="product-info">
								<div class="pic"><img v-if="scope.row.lecturer.image"
										v-img-url="scope.row.lecturer.image.file_path" alt="" /></div>
								<div class="info">
									<div class="name">{{scope.row.lecturer.name}}</div>
								</div>
							</div>
						</template>
					</el-table-column>
					...
					<el-table-column fixed="right" label="操作" width="120">
						<template #default="scope">
							<el-button @click="detailClick(scope.row)" type="text" size="small"
								v-auth="'/lecturer/comment/detail'">详情
							</el-button>
							<el-button @click="delClick(scope.row)" type="text" size="small"
								v-auth="'/lecturer/comment/delete'">删除
							</el-button>
						</template>
					</el-table-column>
				</el-table>
	...
	export default {
		...
		methods: {
			...
			getData(param = '') {
				...
				LecturerApi.getCommentList(Params, true).then(res => {
					self.loading = false;
			...
			onSubmit() {
				...
				Params.page = self.curPage;
				Params.list_rows = self.pageSize;
				LecturerApi.getCommentList(Params, true).then(data => {
					self.loading = false;
