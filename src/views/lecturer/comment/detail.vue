<template>
	...
	<div class="evaluation-detail pb50" v-loading="loading">
		...
		<el-form size="small" ref="form" :model="form" label-width="120px">
			<!--添加门店-->
			<div class="common-form">讲师评价详情</div>
			<el-form-item label="用户">
				<p v-if="form.user">{{ form.user.nickName }}</p>
			</el-form-item>
			<el-form-item label="讲师名称">
				<p>{{ form.lecturer.name }}</p>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			getComment() {
				...
				LecturerApi.getComment({
					...
					.then(data => {
						...
						self.path = data.data.data.lecturer.image ? data.data.data.lecturer.image.file_path : '';
			...
			onSubmit() {
				...
				LecturerApi.editComment(params, true)
					.then(data => {
						if (data.code == 1) {
							ElMessage({
								message: '恭喜你，操作成功',
