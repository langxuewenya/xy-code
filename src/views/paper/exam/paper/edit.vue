<template>
	<div class="product-add" v-loading="loading">
		...
		<el-form size="small" ref="form" :model="form" label-width="180px" v-if="!loading">
			...
			<!--价格设置-->
			<OtherIndex v-show="activeName == 'other'"></OtherIndex>
			<!--审核-->
...
<script>
	...
	import OtherIndex from './part/Other.vue';
	export default {
			...
			onSubmit: function(submit_type) {
				...
				self.$refs.form.validate(valid => {
					if (valid) {
						...
						params.question = [...self.form.model.single, ...self.form.model.many, ...self.form
							.model.judge
						];
						self.save_loading = true;
						params = JSON.stringify(params);
						PaperApi.editexamPaper({
								params: params
							}, true)
							.then(data => {
