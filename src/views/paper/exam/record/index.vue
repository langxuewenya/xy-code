			...
			<div class="table-wrap">
				<el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
					...
					<el-table-column prop="paper.category.name" label="考试分类">
						<template #default="scope">
							<span
								v-if="scope.row.paper&&scope.row.paper.category">{{scope.row.paper.category.name}}</span>
						</template>
					</el-table-column>
					...
					<el-table-column fixed="right" label="操作" width="90">
						<template #default="scope">
							<el-button @click="viewClick(scope.row)" type="text" size="small"
								v-auth="'/paper/exam/record/question'">
								查看答题
							</el-button>
						</template>
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getData() {
				...
				self.loading = true;
				PaperApi.examRecordList(Params, true)
					.then(data => {
						self.loading = false;
			...
			viewClick(row) {
				this.$router.push({
					path: '/paper/exam/record/question',
					query: {
