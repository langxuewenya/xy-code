	...
	<div class="user">
		...
		<div class="common-seach-wrap">
			<el-form size="small" :inline="true" :model="searchForm" class="demo-form-inline">
				...
				<el-form-item>
					<el-button size="small" type="success" @click="onExport" v-auth="'/order/order/export'">导出</el-button>
				</el-form-item>
			</el-form>
		</div>
		<!--内容-->
				...
				<el-table size="small" :data="tableData.data" border style="width: 100%" v-loading="loading">
					<el-table-column prop="order_no" label="订单号" width="150">
						<template #default="scope">
							<div>
								<div class="state-text" :class="{'state-text-red':scope.row.order_source != 10}">
									【{{scope.row.order_source_text}}】</div>
					...
					<el-table-column prop="" label="商品信息" width="400">
						<template #default="scope">
							<div class="product-info" v-for="(item, index) in scope.row.product" :key="index">
								<div class="pic">
									<img v-if="item.image && item.image.file_path" v-img-url="item.image.file_path"
										alt="" />
								</div>
								<div class="info">
					...
					<el-table-column fixed="right" label="操作" width="100">
						<template #default="scope">
							<el-button @click="addClick(scope.row)" type="text" size="small"
								v-auth="'/order/order/detail'">订单详情</el-button>
						</template>
					</el-table-column>
				</el-table>
			</div>
	...
	export default {
		...
		methods: {
			...
			onExport: function() {
				...
				window.location.href = baseUrl + '/index.php/shop/order.order/export?' + qs.stringify(this
					.searchForm);
			},
		}
	};
