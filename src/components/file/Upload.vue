	...
	<div class="upload-wrap">
		<el-dialog title="文件管理" v-model="dialogVisible" :close-on-click-modal="false" custom-class="upload-dialog"
			...
			<div class="fileContainer">
				<div class="file-type">
					<ul>
						<li :class="activeType==item.group_id?'item active':'item'" v-for="(item,index) in typeList"
							:key="index" @click="selectTypeFunc(item.group_id)">
							{{item.group_name}}
							<div class="operation" @click.stop v-if="item.group_id!=null">
								<p @click="editCategoryFunc(item)"><el-icon>
										<Edit />
									</el-icon></p>
								<p @click="deleteCategoryFunc(item.group_id)"><el-icon>
										<Delete />
									</el-icon></p>
							</div>
						</li>
				...
				<div class="file-content">
					<ul class="fileContainerUI clearfix">
						<li class="file" v-for="(item, index) in fileList.data" :key="index"
							...
							<div v-if="item.selected==true" class="selectedIcon"><el-icon>
									<Check />
								</el-icon></div>
							<img v-if="this_config.file_type == 'image'" :scr="item.file_path"
								:style="'background-image:url('+item.file_path+')'" alt="" />
							<video v-if="this_config.file_type == 'video'" height="100" width="100"
								:src="item.file_path" :autoplay="false">
								您的浏览器不支持 video 标签
							</video>
							<p class="desc">{{ item.real_name }}</p>
							<div class="bottom-shade" @click.stop>
								<a href="javascript:void(0)" @click="deleteFileFunc(item)"><el-icon>
										<Delete />
									</el-icon></a>
							</div>
						</li>
			...
			<div class="pagination-wrap">
				<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
					:current-page="curPage" :page-sizes="[12, 24, 36, 42, 48]" :page-size="pageSize"
